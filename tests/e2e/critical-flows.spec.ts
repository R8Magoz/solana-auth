...
async function goToView(page: Page, name: 'Panel' | 'Gastos' | 'Aprobaciones' | 'Informes') {
...
async function goToProfile(page: Page) {
  await sidebar(page).getByText('Mi perfil', { exact: true }).click();
  await page.waitForLoadState('networkidle');
}

async function openNewExpensePanel(page: Page) {
  await goToView(page, 'Gastos');
  const newExpenseButton = page.getByRole('button', { name: 'Nuevo gasto' });
  await expect(newExpenseButton).toBeVisible();
  await newExpenseButton.click();
  await expect(page.locator('.panel-slide .expense-form-fields-wrap').last()).toBeVisible();
}

async function openNewInvoicePanel(page: Page) {
  await goToView(page, 'Panel');
  const newInvoiceCard = page.getByText('Nueva factura', { exact: true }).first();
  await expect(newInvoiceCard).toBeVisible();
  await newInvoiceCard.click();
  await expect(page.getByRole('heading', { name: 'Gastos' })).toBeVisible();
  await expect(page.locator('.panel-slide').getByText('Nueva factura', { exact: true })).toBeVisible();
}

function formWrap(page: Page) {
  return page.locator('.panel-slide .expense-form-fields-wrap').last();
}

function expenseRow(page: Page, text: string) {
  return page.locator('.row-hover').filter({ hasText: text }).first();
}
...
async function createBillViaUi(page: Page, name: string, amount: string) {
  ...
  await expect(expenseRow(page, name)).toBeVisible();
}
...
test.describe('Critical business flows', () => {
  ...
  test('A4) User edits rejected gasto — all fields changeable — resubmits', async ({ page }) => {
    ...
    await loginAs(page, 'user@solana.test');
    await goToView(page, 'Gastos');
    await expenseRow(page, 'Original A4').click();
  ...
  test('A5) Approved gasto cannot be edited by regular user', async ({ page }) => {
    ...
    await loginAs(page, 'user@solana.test');
    await goToView(page, 'Gastos');
    await expenseRow(page, 'Flow A5 aprobado').click();
  ...
  test('B1) Submit factura without A pagar — paymentStatus is paid on creation', async ({ page }) => {
    ...
    await goToView(page, 'Gastos');
    await expenseRow(page, 'Factura B1 sin defer').click();
  ...
  test('C2) Regular user cannot see Aprobar/Rechazar buttons', async ({ page }) => {
    ...
    await loginAs(page, 'user@solana.test');
    await goToView(page, 'Gastos');
    await expenseRow(page, 'C2 pendiente otros').click();
  ...
  test('C3) Regular user can access Mi perfil and change password', async ({ page }) => {
    ...
    await expect(page.getByText('Contraseña actual', { exact: true }).locator('..').locator('input')).toBeVisible();
  ...
  test('C5) Assigned approver sees Aprobar/Rechazar buttons', async ({ page }) => {
    ...
    await wrap.locator('label:has-text("Categoría") + select').first().selectOption({ value: 'Supplies' });
    await wrap.locator('label:has-text("Departamento") + select').first().selectOption({ index: 1 });
    await page.getByRole('button', { name: 'Enviar gasto' }).click();
  ...
  test('F1) Draft persists when navigating away mid-form', async ({ page }) => {
    ...
    await goToView(page, 'Panel');
    await openNewExpensePanel(page);
    await expect(formWrap(page).getByPlaceholder('Concepto')).toHaveValue('PERSIST DRAFT X');
  });
  ...
  test('F2) Draft clears on successful submit', async ({ page }) => {
    ...
    await openNewExpensePanel(page);
    await expect(formWrap(page).getByPlaceholder('Concepto')).toHaveValue('');
  });
});
