        ...
        try{
          ...
          if(receipt?.b64){
            try{
              ...
              try{
                const refreshed=await API.get("/expenses/"+encodeURIComponent(exp.id));
                if(refreshed&&refreshed.expense){
                  const updated=expenseFromApi(refreshed.expense);
                  saveExp(prev => prev.map(x => x.id === updated.id ? updated : x)
                    .concat(prev.find(x => x.id === updated.id) ? [] : [updated]));
                  exp=updated;
                }
              }catch(_){}
